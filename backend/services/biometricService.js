...
class BiometricService {
  ...
  async findEmployeeByDeviceId(deviceUserId, tenantId) {
    try {
      ...
      // Query real database for employee with matching device ID
      // First, try to find user with biometric device ID in their profile
      const user = await User.findOne({
        orgId: tenantId,
        isActive: true,
        'profile.biometricDeviceId': deviceUserId
      }).lean();
      ...
      // If not found by biometric device ID, try by employee code
      const employeeByCode = await Employee.findOne({
        employeeCode: deviceUserId,
        orgId: tenantId,
        status: 'active'
      }).populate('userId', 'name email isActive').lean();

      if (employeeByCode && employeeByCode.userId?.isActive) {
        return {
          id: employeeByCode._id.toString(),
          userId: employeeByCode.userId._id.toString(),
          name: employeeByCode.userId.name,
          email: employeeByCode.userId.email,
          employeeCode: employeeByCode.employeeCode,
          department: employeeByCode.department,
          designation: employeeByCode.designation,
          deviceUserId: deviceUserId,
          tenantId: tenantId
        };
      }

      console.warn(`Employee not found for device ID: ${deviceUserId} in tenant: ${tenantId}`);
      return null;
    } catch (error) {
