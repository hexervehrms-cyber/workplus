...
// Lazy load sales sub-components to reuse existing logic
...
const SalesUnified = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const dashRes = await apiGet('/sales/performance/dashboard', false).catch(() => null);
  ...
  useEffect(() => {
    if (currentTab === 'dashboard' && authUserKey(user)) {
      void fetchDashboardData();
    }
  }, [currentTab, user?.userId, user?.id, fetchDashboardData]);
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Real-time sales performance and metrics</p>
      </div>
      ...
      {/* Leads Tab - Reuse existing Leads component */}
      {currentTab === 'leads' && (
        <Suspense fallback={<div className="text-center py-8">Loading leads...</div>}>
          <Leads />
        </Suspense>
      )}

      {/* Deals Tab - Reuse existing Deals component */}
      {currentTab === 'deals' && (
        <Suspense fallback={<div className="text-center py-8">Loading deals...</div>}>
          <Deals />
        </Suspense>
      )}

      {/* Calls Tab - Reuse existing Calls component */}
      {currentTab === 'calls' && (
