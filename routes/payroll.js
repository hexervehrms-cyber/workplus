 ...
 * Generate payslip with CRITICAL idempotency and race condition prevention
 * P0 CRITICAL FIX: Prevents duplicate payroll processing
 */
router.post('/generate', idempotencyMiddleware, asyncHandler(async (req, res) => {
  const { employeeId, month, year, paidBy, orgId } = req.body;
  ...
  // P0 CRITICAL: Check if payslip already exists for this employee/month/year
  // This prevents duplicate payroll processing
  ...
  // P0 CRITICAL: Use transaction for payslip creation
 ...
 * Bulk generate payslips with idempotency
 * P0 CRITICAL: Prevents duplicate bulk payroll processing
 */
router.post('/bulk-generate', idempotencyMiddleware, asyncHandler(async (req, res) => {
  const { employeeIds, month, year, orgId } = req.body;
  ...
  // P0 CRITICAL: Use transaction for bulk creation
  const session = await mongoose.startSession();
  session.startTransaction();
  ...
  try {
    ...
    logger.info('Bulk payslips generated', {
    ...
    await session.abortTransaction();
    logger.error('Bulk payslip generation failed', { error: error.message });
    throw error;
  } finally {
    session.endSession();
  }
