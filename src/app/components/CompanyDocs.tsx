  ...
  const mockDocuments: CompanyDocument[] = [
    {
      ...
      status: 'Published',
      documentUrl: '/documents/employee_handbook_2024.pdf',
    ...
    },
    {
      ...
      status: 'Published',
      documentUrl: '/documents/code_of_conduct.pdf',
    ...
    },
    {
      ...
      status: 'Published',
      documentUrl: '/documents/it_security_policy.pdf',
    ...
    },
    {
      ...
      status: 'Published',
      documentUrl: '/documents/benefits_guide_2024.pdf',
    ...
    },
    {
      ...
      description: 'Policy and guidelines for remote work arrangements',
      ...
      status: 'Draft',
      documentUrl: '/documents/remote_work_policy.pdf',
  ...
  const loadDocuments = async () => {
    try {
      ...
      if (!isAdmin && !isSuperAdmin) {
        // For employees, fetch user-specific documents
        // const response = await fetch(`/api/company-documents/user/${employeeId}`);
        // const userDocs = await response.json();
        
        // For demo purposes, filter mock documents for employee
        const userDocs = mockDocuments.filter(doc => 
          doc.isPublic && doc.status === 'Published'
        );
        ...
        // const response = await fetch(`/api/company-documents${organizationId ? `?organizationId=${organizationId}` : ''}`);
