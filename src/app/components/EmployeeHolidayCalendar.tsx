  ...
  const loadCalendars = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setCalendars(data);
        // Auto-select the most recent published calendar
        const publishedCalendar = data.find((cal: HolidayCalendar) => cal.isPublished && cal.year === selectedYear);
        if (publishedCalendar) {
          setSelectedCalendar(publishedCalendar);
        }
      }
    } catch (error) {
